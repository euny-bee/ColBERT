...
step1_treccovid.py
------------------
BEIR TREC-COVID 컬렉션에 대해 float16 + 2-bit 인덱스 구축.

실행:
  python step1_treccovid.py
"""
...
COLLECTION   = "D:/beir/trec-covid/collection.tsv"
INDEX_ROOT   = os.path.join(COLBERT_DIR, "experiments/treccovid/indexes")
RAW_EMBS_DIR = "D:/beir/trec-covid/raw_embs"
...
def main():
    log("=" * 55)
    log("step1_treccovid.py - TREC-COVID index build")
    log("=" * 55)

    t_total = time.time()

    log("\n[A] Encoding passages + saving raw embeddings...")
    t0 = time.time()
    rc = subprocess.run(
