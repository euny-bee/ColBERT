...
step2_hotpotqa.py
-----------------
hotpotqa.analog / hotpotqa.2bit 인덱스 검색 + nDCG@10, MRR@10, R@50, R@1k 계산.

실행:
  python step2_hotpotqa.py
"""
...
INDEX_ROOT   = os.path.join(COLBERT_DIR, "experiments/hotpotqa_2.6m/indexes")
RANKING_DIR  = os.path.join(COLBERT_DIR, "experiments/hotpotqa_2.6m/rankings")
RESULTS_FILE = os.path.join(COLBERT_DIR, "experiments/hotpotqa_2.6m/results.json")

QUERIES      = "D:/beir/hotpotqa/queries.test.int.tsv"
QRELS        = "D:/beir/hotpotqa/qrels.test.fair.int.tsv"

ANALOG_INDEX = "hotpotqa.analog"
BIT2_INDEX   = "hotpotqa.2bit"
SEARCH_K     = 1000
NDOCS        = 4096
...
# ============================================================
# 검색 (ranking TSV 직접 저장 - strftime 오류 5 방지)
# ============================================================
def _search(index_name, ranking_path, analog=False):
    ...
    with Run().context(RunConfig(nranks=1, experiment="hotpotqa_2.6m")):
    ...
    # TSV 직접 저장 (strftime 오류 우회): flat_ranking = [(qid, pid, rank, score), ...]
...
def main():
    ...
    analog_ranking = os.path.join(RANKING_DIR, "hotpotqa.analog.tsv")
    rc = subprocess.run(
    ...
    if rc != 0:
        ...
        results["float16"] = m
        log(f"  float16  nDCG@10={m['nDCG@10']}  MRR@10={m['MRR@10']}  R@50={m['R@50']}  R@1k={m['R@1k']}  (n={m['num_queries']})")

    # --- 2-bit 검색
    log("\n[2] Searching 2-bit index...")
    bit2_ranking = os.path.join(RANKING_DIR, "hotpotqa.2bit.tsv")
    rc = subprocess.run(
    ...
    if rc != 0:
        ...
        results["2bit"] = m
        log(f"  2-bit    nDCG@10={m['nDCG@10']}  MRR@10={m['MRR@10']}  R@50={m['R@50']}  R@1k={m['R@1k']}  (n={m['num_queries']})")
    ...
    print(f"\n{'='*65}")
    print(f"  HotpotQA 2.6M - float16 vs 2-bit")
    print(f"{'='*65}")
