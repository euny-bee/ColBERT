...
step1_hotpotqa.py
-----------------
BEIR-HotpotQA 2.6M fair 컬렉션에 대해 float16 + 2-bit 인덱스 구축.

순서:
  A. ColBERT Indexer 실행 → raw float16 embeddings 청크별 저장
  B. float16 index: (emb - centroid).half() → hotpotqa.analog/
  C. 2-bit index: binarize(residual_f16) → hotpotqa.2bit/

실행:
  python step1_hotpotqa.py
"""
...
COLLECTION   = "D:/beir/hotpotqa/collection_2.6m_fair.tsv"
INDEX_ROOT   = os.path.join(COLBERT_DIR, "experiments/hotpotqa_2.6m/indexes")
RAW_EMBS_DIR = "D:/beir/hotpotqa/raw_embs"
...
# ============================================================
# Step C: 2-bit index 구축
# ============================================================
def _build_2bit():
    analog_path = os.path.join(INDEX_ROOT, ANALOG_INDEX)
    bit2_path   = os.path.join(INDEX_ROOT, BIT2_INDEX)

    # 기존 2-bit 인덱스 삭제 (오류 4 방지)
...
def main():
    log("=" * 55)
    log("step1_hotpotqa.py - HotpotQA 2.6M index build")
    log("=" * 55)

    t_total = time.time()

    # A: 인코딩
    log("\n[A] Encoding 2.6M passages + saving raw embeddings...")
    t0 = time.time()
    rc = subprocess.run(
