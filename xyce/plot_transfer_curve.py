...
# --- OFF fit: log10(Ids) = a*Vgs^2 + b*Vgs + c  (log 도메인 2차) ---
log_ids_off = np.log10(ids_off)
coeffs_off  = np.polyfit(vgs_off, log_ids_off, 2)
poly_off    = np.poly1d(coeffs_off)

r2_off = 1 - np.sum((log_ids_off - poly_off(vgs_off))**2) / \
             np.sum((log_ids_off - log_ids_off.mean())**2)
...
# --- 연결점에서 continuity 보정: OFF 상수항을 ON 값에 맞춤 ---
y_on_at_boundary  = poly_on(V_BOUNDARY)
y_off_at_boundary = poly_off(V_BOUNDARY)
coeffs_off[-1] += (y_on_at_boundary - y_off_at_boundary)
poly_off = np.poly1d(coeffs_off)

# --- Fit 선 생성 ---
vgs_off_fit = np.linspace(vgs.min(), V_BOUNDARY, 300)
ids_off_fit = 10 ** poly_off(vgs_off_fit)
...
print(f"\n[OFF]  log10(Ids) = {coeffs_off[0]:.4f}·Vgs² + {coeffs_off[1]:.4f}·Vgs + ({coeffs_off[2]:.4f})")
print(f"       R² = {r2_off:.4f}")
print(f"\n[ON]   log10(Ids) = {coeffs_on[0]:.4f}·Vgs³ + {coeffs_on[1]:.4f}·Vgs²"
      f" + {coeffs_on[2]:.4f}·Vgs + ({coeffs_on[3]:.4f})")
print(f"       R² = {r2_on:.4f}")
print(f"\n연결점 Ids: OFF={10**poly_off(V_BOUNDARY):.3e}, ON={10**poly_on(V_BOUNDARY):.3e}")
...
ax.semilogy(vgs_off_fit, ids_off_fit, color="steelblue", linewidth=2.5,
            label=f"OFF fit (2nd)  R²={r2_off:.3f}")
...
off_eq = (f"OFF ($V_{{GS}} < {V_BOUNDARY}$ V):\n"
          f"$\\log_{{10}}(I_{{DS}}) = {coeffs_off[0]:.4f}\\cdot V_{{GS}}^2"
          f" {coeffs_off[1]:+.4f}\\cdot V_{{GS}} ({coeffs_off[2]:+.4f})$\n"
          f"$R^2 = {r2_off:.3f}$")
...
ax.text(0.02, 0.76, on_eq, transform=ax.transAxes, fontsize=8.5,
        verticalalignment="top", color="tomato",
        bbox=dict(boxstyle="round,pad=0.3", facecolor="white", alpha=0.8))

plt.tight_layout()
plt.savefig("transfer_curve_connected.png", dpi=150)
plt.show()
print("\nSaved: transfer_curve_connected.png")
