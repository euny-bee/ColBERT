"""
plot_combined_split_newline_fixedcell.py의 row0 scatter에서 q1 패널만 제거한 버전.
기존 [vth_v3_fixedcell]row0_scatter_newline.png(3-panel)는 그대로 두고 새 파일로 저장.
"""
import matplotlib
matplotlib.use('Agg')
import pandas as pd
...
Q_LABELS = {
    0: 'q0: "...house reshingled" (margin=1.62, tight race)',
    2: 'q2: "where is hartwell ga" (margin=20.06, dominant win)',
}
...
print("Row 0 (scatter, q1 제외) 그리는 중...")
fig0, axes0 = plt.subplots(1, 2, figsize=(12, 5))

for panel_idx, q_id in enumerate([0, 2]):
    ax       = axes0[panel_idx]
    ...
    if panel_idx == 0:
        ...
        ], fontsize=12, loc='upper right', title="Ranking\nComparison", title_fontsize=13)

fig0.tight_layout()
out0 = f'{BASE}/[vth_v3_fixedcell]row0_scatter_newline_no_q1.png'
