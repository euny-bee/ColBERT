...
Output: table_periphery_full_with_write.png
"""
...
OUTPUT     = SCRIPT_DIR / "table_periphery_full_with_write.png"
...
TITLE = "System-level scale (2,048 centroids + 255 queries + 10,988 documents)"
...
rows = [
    ...
    ("+ TIA",                "19,045.87 nJ",  "+12,289.33 nJ", "31,335.20 nJ",  "3.408 TOPS/W",  "126.1×"),
    ("+ ADC",                "19,671.60 nJ",  "+12,289.33 nJ", "31,960.93 nJ",  "3.341 TOPS/W",  "123.6×"),
    ("+ Driver",             "209,233.20 nJ", "+12,289.33 nJ", "221,522.53 nJ", "0.4821 TOPS/W", "17.8×"),
    ("+ ML buffer (final)",  "296,835.12 nJ", "+12,289.33 nJ", "309,124.45 nJ", "0.3454 TOPS/W", "12.8×"),
]

COL_X   = [0.02]
COL_NUM = [0.375, 0.525, 0.675, 0.81, 0.935]
...
ax.text(0.0, y_bottom - 0.028,
        "Table 4-Full-Periphery + Write (scope-consistent): cumulative full-periphery search energy/query\n"
        "(cell -> +TIA -> +ADC -> +driver -> +ML buffer, Option A) plus one-time write-phase energy (index build:\n"
        "cell + row/col write-driver, Option A) amortized over an assumed index lifetime of 255 queries.\n"
        "Write total: 3,133,780.0 nJ (2,048 centroids, 10,988 documents). Search-only cumulative values from\n"
        "periphery_energy_model_v2.py. Both search and write include their own row/col driver here, so vs.-GPU\n"
        "is scope-consistent (unlike the headline-only + write companion table).",
