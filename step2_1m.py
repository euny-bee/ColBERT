...
step2_1m.py
-----------
1m.analog / 1m.2bit 인덱스 검색 + MRR@10, R@50, R@1k 계산.
...
RANKING_DIR  = os.path.join(COLBERT_DIR, "experiments/msmarco_1m/rankings")
RESULTS_FILE = os.path.join(COLBERT_DIR, "experiments/msmarco_1m/results.json")

QUERIES      = os.path.join(COLBERT_DIR, "data/msmarco/queries.dev.tsv")
...
def main():
    ...
    analog_ranking = os.path.join(RANKING_DIR, "1m.analog.tsv")
    rc = subprocess.run(
    ...
    if rc != 0:
        ...
        results["float16"] = m
        log(f"  float16  MRR@10={m['MRR@10']}  R@50={m['R@50']}  R@1k={m['R@1k']}  (n={m['num_queries']})")

    # --- 2-bit 검색
    log("\n[2] Searching 2-bit index...")
    bit2_ranking = os.path.join(RANKING_DIR, "1m.2bit.tsv")
    rc = subprocess.run(
    ...
    if rc != 0:
        ...
        results["2bit"] = m
        log(f"  2-bit    MRR@10={m['MRR@10']}  R@50={m['R@50']}  R@1k={m['R@1k']}  (n={m['num_queries']})")
    ...
    print(f"\n{'='*58}")
    print(f"  MS MARCO 1M - float16 vs 2-bit")
    print(f"{'='*58}")
