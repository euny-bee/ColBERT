...
----------------------
200K MS MARCO 서브셋에 대해 float16 + 2-bit 인덱스 구축.

순서:
  A. ColBERT Indexer 실행 → raw float16 embeddings 청크별 저장
  B. float16 index: (emb - centroid).half() → 200k.analog/
  C. 2-bit index: binarize(residual_f16) → 200k.2bit/
...
RAW_EMBS_DIR = os.path.join(COLBERT_DIR, "experiments/msmarco/raw_embs")
...
# ============================================================
def _build_analog():
    """raw embs → (emb - centroid).half() → 200k.analog 잔차 교체."""
    ...
    for chunk_file in chunk_files:
        ...
        for sub in embs.split(1 << 16):  # 65K 단위 처리
            ...
            residuals = sub.half() - centroids.half()   # float16
...
def _build_2bit():
    ...
    bounds = torch.cat([
        torch.tensor([-1e4]), bucket_cutoffs, torch.tensor([1e4])
    ])
    ...
    # buckets.pt 업데이트 — ColBERT는 tuple (cutoffs, weights) 형식으로 저장
    torch.save(
    ...
    for rf in residual_files:
        ...
        res_f16 = torch.load(rf, map_location="cpu", weights_only=True)  # (N, 128) float16
...
def main():
    log("=" * 55)
    log("step1_build_indexes.py")
    log("=" * 55)

    # A: 인코딩 (subprocess → multiprocessing 격리)
    log("\n[A] Encoding 200K passages + saving raw embeddings...")
    rc = subprocess.run(
    ...
    # C: 2-bit-from-float16 index
    log("\n[C] Building 2-bit-from-float16 index...")
    _build_2bit()
