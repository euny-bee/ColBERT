"""
[clip99.9] ColBERT 아날로그 파이프라인 — Vth mismatch v3 모델
  Option A: M0/M3 각자 vth_stored 보유 → 개별 calibration → Vth 완전 상쇄
  Option C: 보상 없음, M0/M3 Vth 독립 샘플링 ~ TruncGauss[0, 3]V (비율유지)

  v2 대비 변경: VTH_STD 0.15→0.90, VTH_HI 0.5→3.0 (range/std 비율 동일)
"""
...
CLIP   =  1e-14
Vth_orig = 0.151045
VSAT_OD  = VSAT - Vth_orig   # 2.729V — overdrive at velocity saturation (소자 상수)

def I_single(vgs, vth):
    # overdrive 기준 VSAT clip: transfer curve가 Vth만큼 이동해도 포화점도 함께 이동
...
# ==========================================================================
# Vth 분포 — v3: range [0, 3]V, std=0.90 (비율유지: range/std=3.33 동일)
# ==========================================================================
VTH_STD = 0.90
...
Vth_M0_s2 = sample_vth((96, 100))[:, :, np.newaxis]
Vth_M3_s2 = sample_vth((96, 100))[:, :, np.newaxis]
I_optC_uA = cell_optC(d_step2, Vth_M0_s2, Vth_M3_s2).sum(axis=2) * 1e-3
...
def save_step2(top_arr, I_uA, name):
    ...
    out = f'{BASE}/[vth_v3]step2_{name}_centroid.xlsx'
    pd.DataFrame(rows).to_excel(out, index=False)
    print(f"  저장: [vth_v3]step2_{name}_centroid.xlsx")
...
def ivf_lookup(top_arr, method_name):
    ...
    out = f'{BASE}/[vth_v3]step3_{method_name}_candidate_pids.xlsx'
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        for q_name, df_r in results.items():
            df_r.to_excel(w, sheet_name=q_name, index=False)
    print(f"  저장: [vth_v3]step3_{method_name}_candidate_pids.xlsx")
    return results
...
def mincurrent_optC(Q_q, D_pid, vth_m0, vth_m3):
    ...
    I_uA = cell_optC(d,
                     vth_m0[:, :, np.newaxis],
                     vth_m3[:, :, np.newaxis]).sum(axis=2) * 1e-3
    return float(I_uA.min(axis=1).sum())
...
for q_id in [0, 1, 2]:
    ...
    for pid in cands_optC[f'q{q_id}']['pid'].tolist():
        ...
        idxs  = [doc_token_index[k] for k in toks]
        M_tok = len(idxs)
        vth_m0 = sample_vth((32, M_tok))
        vth_m3 = sample_vth((32, M_tok))
...
# 저장
# ==========================================================================
out_path = f'{BASE}/[vth_v3]step6_all_results.xlsx'
...
print(f"\n저장: [vth_v3]step6_all_results.xlsx")
print("\nPipeline 완료!")
