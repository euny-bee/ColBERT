"""
newq_margin2 연구용 데이터셋 구축 (Step 1~7) -- 신규 3-query, margin 타이트(1.76~2.01)
...
top2 폴더: 쿼리당 경쟁 passage 1개(rank2)만 강제 포함 (find_competitors_newq_margin2.py 산출,
20k-pool IVF 후보 중 true 이하 최고점 경쟁자 top-10에서 앞 1개만 사용)
...
COMP_JSON  = r'C:\Users\nmdl-khb\ColBERT\centroidset_vector 크기 조절\08_newq_margin2\newq_margin2_top10_competitors.json'
COLLECTION = r'D:\msmarco\collection_1m.tsv'
N_LINES    = 1_100_000
N_DOCS     = 200
N_CENT     = 100
SEED       = 42
TOP_N      = 10   # 강제 포함 경쟁자 개수 (top10 리스트에서 앞 TOP_N-1개 사용, rank2~rank{TOP_N})

QUERY_QIDS = [581521, 579133, 690508]     # -> q0, q1, q2
TRUE_PIDS  = [493988, 303786, 178693]     # 각 qid의 true passage (같은 순서)

with open(COMP_JSON, encoding='utf-8') as f:
    all_competitors = json.load(f)
COMPETITORS = {int(qid): pids[:TOP_N - 1] for qid, pids in all_competitors.items()}
COMPETITOR_PIDS = sorted({p for pids in COMPETITORS.values() for p in pids})
print(f"경쟁 passage 총 {len(COMPETITOR_PIDS)}개 (query당 top-{TOP_N-1}, 중복 없음)")
...
# ==========================================================================
# Step 2: distractor 선정 + 200-doc pool 구성
# ==========================================================================
print(f"\n[Step 2] Distractor 샘플링 중 (query당 top-{TOP_N-1} 경쟁 passage 강제 포함)...")
required = TRUE_PIDS + COMPETITOR_PIDS
random.seed(SEED)
...
print(f"  pool 크기: {len(pool_pids)}  (true={len(TRUE_PIDS)}, competitors={len(COMPETITOR_PIDS)}, random distractor={len(distractors)})")
