"""
[vth_v2] 비교 시각화 — centroidset의 4종 그래프를 동일 구조로 재현
  1. compare_candidates_all   : 3-way 후보 집합 비교 (bar + scatter)
  2. compare_ranking_L2_optA  : Digital L2 vs Option A rank scatter + bump
  3. compare_ranking_L2_optC  : Digital L2 vs Option C rank scatter + bump
  4. compare_ranking_all3     : Digital / Option A / Option C 3-way rank 비교
"""
...
# =============================================================================
print("데이터 로드 중...")
dig_sheets  = pd.read_excel(f'{BASE}/[vth_v2]step3_digital_candidate_pids.xlsx', sheet_name=None)
optA_sheets = pd.read_excel(f'{BASE}/[vth_v2]step3_optA_candidate_pids.xlsx',   sheet_name=None)
optC_sheets = pd.read_excel(f'{BASE}/[vth_v2]step3_optC_candidate_pids.xlsx',   sheet_name=None)
step6       = pd.read_excel(f'{BASE}/[vth_v2]step6_all_results.xlsx',            sheet_name=None)

# step6 시트: q{id}_digital / q{id}_optA / q{id}_optC
...
fig.suptitle(
    "[vth_v2]  Digital vs OptionA & OptionC  —  Candidate Set Comparison  (nprobe=2)\n"
    "OptionA: 개별 Vth 보상  |  OptionC: M0/M3 독립 Vth ~ TruncGauss[0,0.5]V, seed=42",
...
def draw_scatter(ax, dig_pids, other_pids, dig_df, other_df, label_other, q_id, other_color):
    ...
    if inter:
        ...
        ax.scatter(dig_df.loc[cp, 'n_tokens'].values,
                   ...
                   label=f'Common ({len(cp)})')
    if dig_only:
        dp = list(dig_only)
        ax.scatter(dig_df.loc[dp, 'n_tokens'].values,
                   np.zeros(len(dp)), alpha=0.5, s=25,
                   color=COLORS['dig_only'], marker='^',
                   label=f'Digital only ({len(dp)})')
    if other_only:
        op = list(other_only)
        ax.scatter(np.zeros(len(op)),
                   other_df.loc[op, 'n_tokens'].values,
...
plt.tight_layout()
out = f'{BASE}/[vth_v2]compare_candidates_all.png'
...
def plot_rank_comparison(df_x_dict, df_y_dict, x_rank_col, y_rank_col,
    ...
    for q_id in [0, 1, 2]:
        ...
        ax.legend(handles=[
            ...
            mlines.Line2D([], [], color=COLORS['true'],   marker='*', ls='None', ms=10, label=f'True pid ({true_pid})'),
        ...
        for pid in all_p:
            ...
            y_only    = (yr_ is not None and yr_ <= TOP_K) and (dr is None or dr > TOP_K)
...
plot_rank_comparison(
    ...
    title=("[vth_v2]  Digital L2  vs  Option A  —  Ranking Comparison\n"
           "OptionA: M0/M3 개별 Vth 보상 → Vth 완전 상쇄"),
    out_path=f'{BASE}/[vth_v2]compare_ranking_L2_optA.png',
...
plot_rank_comparison(
    ...
    title=("[vth_v2]  Digital L2  vs  Option C  —  Ranking Comparison\n"
           "OptionC: M0/M3 독립 Vth ~ TruncGauss[0,0.5]V, seed=42"),
    out_path=f'{BASE}/[vth_v2]compare_ranking_L2_optC.png',
...
fig.suptitle(
    "[vth_v2]  Digital L2  vs  Option A  vs  Option C  —  3-way Ranking Comparison\n"
    "OptionA: 개별 Vth 보상  |  OptionC: M0/M3 독립 Vth ~ TruncGauss[0,0.5]V, seed=42",
...
# 상단: scatter (Digital x, optA/C y 동시)
for q_id in [0, 1, 2]:
    ax       = axes[0, q_id]
    true_pid = QUERY_RELEVANT[q_id]
...
plt.tight_layout()
out = f'{BASE}/[vth_v2]compare_ranking_all3.png'
plt.savefig(out, dpi=150, bbox_inches='tight')
print(f"  저장: {out}")
plt.close()

print("\n완료! 생성된 파일:")
for name in ['[vth_v2]compare_candidates_all.png',
             '[vth_v2]compare_ranking_L2_optA.png',
             '[vth_v2]compare_ranking_L2_optC.png',
             '[vth_v2]compare_ranking_all3.png']:
    print(f"  {BASE}/{name}")
