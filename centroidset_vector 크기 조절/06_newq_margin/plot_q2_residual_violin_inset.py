"""
q2 잔차(residual = analog rank - digital rank) 분포 비교 그림 중 '왼쪽 패널'(violin+box+jitter)만
inset용으로 세로로 좁게(가로 50%, 70%) 두 버전으로 저장하고, 범례(legend)는 별도 이미지로 저장.
"""
import matplotlib
matplotlib.use('Agg')
import pandas as pd
...
draw_violin_panel(0.5, f'{OUT_BASE}/[newq_margin_seed19]q2_residual_violin_w50.png',
                   title_fs=11, ylabel_fs=11 * 1.2 * 1.2 * 1.2 * 1.2, title_wrap=True)
draw_violin_panel(0.7, f'{OUT_BASE}/[newq_margin_seed19]q2_residual_violin_w70.png',
...
leg_out = f'{OUT_BASE}/[newq_margin_seed19]q2_residual_legend.png'
...
stats_out = f'{OUT_BASE}/[newq_margin_seed19]q2_residual_stats_box.png'
...
draw_stats_box('0.6', f'{OUT_BASE}/[newq_margin_seed19]q2_residual_stats_box_no_levene.png')
draw_stats_box('black', f'{OUT_BASE}/[newq_margin_seed19]q2_residual_stats_box_no_levene_blackedge.png')
