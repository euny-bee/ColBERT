"""
newq_margin pool, Vth SEED=19(worst case, q0 No Comp rank=8) 결과를 첨부 참고 이미지와 동일한
스타일(row0 scatter: Digital rank vs Analog rank, Vth Comp=파란 원, No Comp=주황 다이아몬드, True=별)로 시각화.
출력은 'centroidset_vector 크기 조절' 루트 폴더에 저장.
"""
import matplotlib
matplotlib.use('Agg')
import pandas as pd
...
print("데이터 로드 중...")
dig_sheets  = pd.read_excel(f'{IN_BASE}/[seed19]step3_digital_candidate_pids.xlsx', sheet_name=None)
optA_sheets = pd.read_excel(f'{IN_BASE}/[seed19]step3_optA_candidate_pids.xlsx',   sheet_name=None)
optC_sheets = pd.read_excel(f'{IN_BASE}/[seed19]step3_optC_candidate_pids.xlsx',   sheet_name=None)
...
fig0.tight_layout()
out0 = f'{OUT_BASE}/[newq_margin_seed19]row0_scatter_newline.png'
