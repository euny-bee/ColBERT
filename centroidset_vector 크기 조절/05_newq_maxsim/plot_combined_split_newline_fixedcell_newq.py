"""
plot_combined_split_newline_fixedcell.py 의 newq(신규 3-query) 버전.
  [newq_clip99.9]/[newq_vth_v3_fixedcell] 파일을 읽어
  [newq_vth_v3_fixedcell]row0_scatter_newline.png
  [newq_vth_v3_fixedcell]row1_bar.png
  기존 [vth_v3_fixedcell] 파일/figure는 건드리지 않음.
"""
...
Q_LABELS = [
    'q0: "what is the main goal of sociology" (low maxsim)',
    'q1: "head sores scalp" (median maxsim)',
    'q2: "what is the population of pakistan" (high maxsim)',
]
TOP_K = 30
...
# ── 데이터 로드 ────────────────────────────────────────────────────────────────
print("데이터 로드 중...")
dig_sheets  = pd.read_excel(f'{BASE}/[newq_vth_v3_fixedcell]step3_digital_candidate_pids.xlsx', sheet_name=None)
optA_sheets = pd.read_excel(f'{BASE}/[newq_vth_v3_fixedcell]step3_optA_candidate_pids.xlsx',   sheet_name=None)
optC_sheets = pd.read_excel(f'{BASE}/[newq_vth_v3_fixedcell]step3_optC_candidate_pids.xlsx',   sheet_name=None)
step6       = pd.read_excel(f'{BASE}/[newq_vth_v3_fixedcell]step6_all_results.xlsx',            sheet_name=None)
...
for panel_idx, q_id in enumerate([0, 1, 2]):
    ...
    xlim = max(130, max(true_xs, default=0) + 10)
    ax.plot([0, xlim], [0, xlim], 'k--', lw=0.8, alpha=0.3)
...
fig0.tight_layout()
out0 = f'{BASE}/[newq_vth_v3_fixedcell]row0_scatter_newline.png'
...
for panel_idx, q_id in enumerate([0, 1, 2]):
    ...
    ax.set_title(Q_LABELS[q_id], fontsize=7.5, fontweight='bold')
...
fig1.tight_layout()
out1 = f'{BASE}/[newq_vth_v3_fixedcell]row1_bar.png'
