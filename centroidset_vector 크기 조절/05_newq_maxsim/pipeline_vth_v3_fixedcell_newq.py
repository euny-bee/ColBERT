"""
[newq_clip99.9] ColBERT 아날로그 파이프라인 — Vth mismatch v3 모델, "셀단위 고정 Vth" 버전 (1+2 조합)

  pipeline_vth_v3_fixedcell.py 의 newq(신규 3-query) 버전.
  q0/q1/q2 를 maxsim(query, true passage) 값이 낮음/중간/높음인 새 3개 query로 교체:
    q0: qid 829425 "what is the main goal of sociology"   true_pid=986805  (maxsim 낮음)
    q1: qid 199572 "head sores scalp"                     true_pid=378086  (maxsim 중간)
    q2: qid 840053 "what is the population of pakistan"   true_pid=529990  (maxsim 높음)

  출력: [newq_vth_v3_fixedcell] 접두사로 저장 -- 기존 [vth_v3_fixedcell] 파일은 그대로 보존
"""
...
BASE           = r'C:\Users\nmdl-khb\ColBERT\centroidset_vector 크기 조절\05_newq_maxsim'
NPROBE         = 2
QUERY_RELEVANT = {0: 986805, 1: 378086, 2: 529990}
SEED           = 42
...
# ==========================================================================
print("데이터 로드 중...")
Q_df = pd.read_excel(f'{BASE}/[newq_clip99.9]query_embs_96x128.xlsx',  index_col=0)
C_df = pd.read_excel(f'{BASE}/[newq_clip99.9]centroids_100x128.xlsx',  index_col=0)
doc  = pd.read_excel(f'{BASE}/[newq_clip99.9]doc_embs_13268x128.xlsx', index_col=0)
ivf  = pd.read_excel(f'{BASE}/[newq_clip99.9]ivf_centroid2pid.xlsx',
                     sheet_name='long_format', index_col=0)
qcr  = pd.read_excel(f'{BASE}/[newq_clip99.9]query_centroid_ranking.xlsx',
...
def save_step2(top_arr, I_uA, name):
    ...
    out = f'{BASE}/[newq_vth_v3_fixedcell]step2_{name}_centroid.xlsx'
    pd.DataFrame(rows).to_excel(out, index=False)
    print(f"  저장: [newq_vth_v3_fixedcell]step2_{name}_centroid.xlsx")
...
def ivf_lookup(top_arr, method_name):
    ...
    out = f'{BASE}/[newq_vth_v3_fixedcell]step3_{method_name}_candidate_pids.xlsx'
    with pd.ExcelWriter(out, engine='openpyxl') as w:
        for q_name, df_r in results.items():
            df_r.to_excel(w, sheet_name=q_name, index=False)
    print(f"  저장: [newq_vth_v3_fixedcell]step3_{method_name}_candidate_pids.xlsx")
    return results
...
# 저장
# ==========================================================================
out_path = f'{BASE}/[newq_vth_v3_fixedcell]step6_all_results.xlsx'
...
print(f"\n저장: [newq_vth_v3_fixedcell]step6_all_results.xlsx")
print("\nPipeline (newq fixedcell) 완료!")
