...
# 각 query별 실제 rank2~rank11(=top-10 경쟁 passage), 500-distractor margin 계산에서 식별
COMPETITORS = {
    329114: [559729, 316414, 488061, 345376, 182389, 516496, 629999, 629316, 929535, 451677],
    498398: [123013, 954448, 214423, 851,    1081229, 591048, 446040, 552522, 134323, 127160],
    984178: [326168, 169045, 765309, 526787, 324736,  926818, 294073, 233556, 584434, 312687],
}
COMPETITOR_PIDS = sorted({p for pids in COMPETITORS.values() for p in pids})
print(f"경쟁 passage 총 {len(COMPETITOR_PIDS)}개 (query당 top-10, 중복 없음)")
...
# ==========================================================================
print("\n[Step 2] Distractor 샘플링 중 (query당 top-10 경쟁 passage 강제 포함)...")
...
print(f"  pool 크기: {len(pool_pids)}  (true={len(TRUE_PIDS)}, top10 competitors={len(COMPETITOR_PIDS)}, random distractor={len(distractors)})")
