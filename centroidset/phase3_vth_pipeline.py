"""
Phase 3 Vth 변이 평가 파이프라인 (Full-GPU 버전)
  Dataset : MS MARCO 255 queries x 50k doc pool
  Methods : Digital | Option A (VthComp) | Option C (NoComp)
  Vth 조건 : PBS shift [0,0.5] / [0,1] / [0,2] / [0,3] V
  출력    : phase3_vth_results.csv  (255 x 4 = 1020 rows)

GPU 최적화:
  - sampler: scipy truncnorm -> torch.erfinv (역 CDF, GPU 직접 샘플링)
  - Step 2: per-query GPU 배치 (32, CBATCH, 128)
  - Step 6: 후보 50개 묶음 GPU 배치 (Python loop 40x 감소)
  - vecs_gpu: 전체 토큰 벡터 GPU 상주
"""
...
VSAT_OD = VSAT - Vth_orig   # 2.729V

def I_single_t(vgs, vth_val):
    """vgs: Tensor, vth_val: float or same-shape Tensor"""
...
# ==========================================================================
# GPU 샘플러: torch.erfinv 역 CDF (scipy 없음)
# ==========================================================================
def make_sampler_gpu(lo, hi, std):
    """TruncNorm[lo, hi] shift -> Vth_orig + shift, 전부 GPU 텐서"""
    ...
    def _sample(shape):
        ...
        return Vth_orig + shift   # GPU Tensor
...
# Step 2
# ==========================================================================
CBATCH_S2 = 2048   # per-query centroid batch: (32, 2048, 128) ~ 32 MB

def s2_digital():
    scores = Q_gpu @ C_gpu.T          # (255, 32, 32768)
    return torch.topk(scores, NPROBE, dim=2, largest=True).indices.cpu().numpy().astype(np.int32)

def s2_analog(cell_fn):
    """cell_fn(d) -> current tensor, per-query centroid batching"""
    ...
    for q in range(NQ):
        ...
        for cb in range(0, NC, CBATCH_S2):
            ...
            d = Qq[:, None, :] - C_gpu[cb:ce][None, :, :]   # (32, cb_size, 128)
...
def s2_optC(vm0_table, vm3_table):
    """Option C: vm0_table/vm3_table (NC, 128) -- 셀(centroid x 차원) 고정 Vth, 전 쿼리 공통"""
    top = np.empty((NQ, 32, NPROBE), dtype=np.int32)
    for q in range(NQ):
        I_q = torch.empty(32, NC, device=DEV)
        Qq = Q_gpu[q]
        for cb in range(0, NC, CBATCH_S2):
            ...
            vm0 = vm0_table[cb:ce].unsqueeze(0)                 # (1, size, 128): 차원별 독립, 쿼리 무관 고정
            vm3 = vm3_table[cb:ce].unsqueeze(0)
...
# ==========================================================================
# Step 6: 후보 PBATCH개 묶음 GPU 배치
# ==========================================================================
PBATCH = 50    # passages per GPU call: (32, ~3400, 128) ~ 56 MB
...
def s6_optC(q_idx, cands, vm0_table, vm3_table):
    ...
    # 쿼리당 1회만 host->GPU 변환 + gather (배치마다 반복하면 훨씬 느려짐)
...
# -- Option C: 4 Vth conditions --
NT = vecs_gpu.shape[0]
DIM = vecs_gpu.shape[1]
print(f"token cells: {NT:,} x {DIM} dims, centroid cells: {NC:,} x {DIM} dims")
...
for cname, lo, hi, std in VTH_CONDS:
    ...
    # 셀(centroid/토큰 x 차원) 고정 Vth 테이블 -- 이 PBS 조건 안에서는 모든 쿼리가 동일한 값을 공유
    vm0_c = sampler((NC, DIM))
    vm3_c = sampler((NC, DIM))
    vm0_t = sampler((NT, DIM))
    vm3_t = sampler((NT, DIM))

    print("  Step 2 -- Option C...", flush=True)
    t1 = time.time()
    top_optC   = s2_optC(vm0_c, vm3_c)
    ...
    optC_res = []
    for q in range(NQ):
        optC_res.append(s6_optC(q, cands_optC[q], vm0_t, vm3_t))
        if (q + 1) % 64 == 0:
            print(f"    {q+1}/{NQ} ({time.time()-t1:.1f}s)", flush=True)
    print(f"  done ({time.time()-t1:.1f}s)")

    del vm0_c, vm3_c, vm0_t, vm3_t
...
out = f'{BASE}/phase3_vth_results.csv'
