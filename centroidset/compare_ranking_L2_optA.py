"""
Digital L2 vs Option A Ranking Comparison
...
for q_id in [0, 1, 2]:
    ...
    if len(rel):
        print(f"  [q{q_id}] Digital L2 정답 rank: {int(rel.iloc[0]['rank_L2'])}/{len(df)}")

# Option A rank 로드
optA_rank = {}
for q_id in [0, 1, 2]:
    df = optA_step6[f'q{q_id}_analog'].set_index('pid')
    optA_rank[q_id] = df
    rel = df[df['is_relevant']]
    if len(rel):
        print(f"  [q{q_id}] Option A 정답 rank: {int(rel.iloc[0]['rank_ana_f32'])}/{len(df)}")
...
fig.suptitle(
    "Digital L2  vs  Option A Analog Current  —  Ranking Comparison\n"
    "(doc vector: float32,  Option A: Vth compensation, fixed Vth=0.151V)",
...
for q_id in [0, 1, 2]:
    ...
    true_pid = QUERY_RELEVANT[q_id]
    df_dig   = dig_L2[q_id]
    df_optA  = optA_rank[q_id]

    common_pids = [p for p in df_dig.index if p in df_optA.index]

    x_ranks, y_ranks, colors, sizes = [], [], [], []
    for pid in common_pids:
        xr = int(df_dig.loc[pid, 'rank_L2'])
        yr = int(df_optA.loc[pid, 'rank_ana_f32'])
        x_ranks.append(xr)
        y_ranks.append(yr)
        colors.append(COLORS['true'] if pid == true_pid else COLORS['common'])
        sizes.append(220 if pid == true_pid else 25)

    ax.scatter(x_ranks, y_ranks, c=colors, s=sizes, alpha=0.6, zorder=3)

    if true_pid in df_dig.index and true_pid in df_optA.index:
        tx = int(df_dig.loc[true_pid, 'rank_L2'])
        ty = int(df_optA.loc[true_pid, 'rank_ana_f32'])
    ...
    ax.set_ylabel('Option A Current rank  (lower = more similar)', fontsize=9)
    ax.set_title(f'{Q_LABELS[q_id]}\ncommon candidates: {len(common_pids)}',
...
for q_id in [0, 1, 2]:
    ...
    df_optA  = optA_rank[q_id].reset_index()

    dig_topk  = df_dig.nsmallest(TOP_K, 'rank_L2')['pid'].tolist()
    optA_topk = df_optA.nsmallest(TOP_K, 'rank_ana_f32')['pid'].tolist()
    all_pids  = list(dict.fromkeys(dig_topk + optA_topk))

    dig_rank_dict  = dict(zip(df_dig['pid'],  df_dig['rank_L2']))
    optA_rank_dict = dict(zip(df_optA['pid'], df_optA['rank_ana_f32']))

    for pid in all_pids:
        dr  = dig_rank_dict.get(pid)
        ar  = optA_rank_dict.get(pid)

        is_true   = (pid == true_pid)
        in_both   = (dr is not None and dr <= TOP_K) and (ar is not None and ar <= TOP_K)
        dig_only  = (dr is not None and dr <= TOP_K) and (ar is None or ar > TOP_K)
        optA_only = (ar is not None and ar <= TOP_K) and (dr is None or dr > TOP_K)

        col  = COLORS['true'] if is_true else (
               COLORS['common']    if in_both   else
               COLORS['dig_only']  if dig_only  else COLORS['optA_only'])
        ...
        if dr is not None and dr <= TOP_K:
            pts.append((0, dr))
        if ar is not None and ar <= TOP_K:
            pts.append((1, ar))
        ...
        if ar is not None and ar <= TOP_K:
            ax.scatter(1, ar, color=col, s=60 if is_true else 20,
                       marker='*' if is_true else 'o', zorder=4, alpha=alph)

        if is_true:
            if dr is not None and dr <= TOP_K:
                ax.annotate(f'True (rank {dr})', (0, dr),
                            textcoords='offset points', xytext=(-68, 0),
                            fontsize=7.5, color=COLORS['true'], fontweight='bold', ha='left')
            if ar is not None and ar <= TOP_K:
                ax.annotate(f'True (rank {ar})', (1, ar),
    ...
    ax.set_xticks([0, 1])
    ax.set_xticklabels(['Digital L2', 'Option A Current'], fontsize=10, fontweight='bold')
    ...
    h_optA  = mlines.Line2D([], [], color=COLORS['optA_only'], lw=2, label='Option A only')
    h_true  = mlines.Line2D([], [], color=COLORS['true'],      lw=2.5, label='True document')
    ax.legend(handles=[h_both, h_dig, h_optA, h_true], fontsize=7.5, loc='lower right')

plt.tight_layout()
out = f'{BASE}/compare_ranking_L2_optA.png'
